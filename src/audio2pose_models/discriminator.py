...
class PoseSequenceDiscriminator2(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        leaky = self.cfg.MODEL.DISCRIMINATOR.LEAKY_RELU
        ...
        self.seq = nn.Sequential(
            ConvNormRelu('1d', cfg.MODEL.DISCRIMINATOR.INPUT_CHANNELS,
                         256, downsample=True, leaky=leaky),  # B, 256, T/2
            ConvNormRelu('1d', 256, 512, downsample=True,
                         leaky=leaky),  # B, 512, T/4
            ConvNormRelu('1d', 512, 1024, kernel_size=3, stride=1,
                         padding=1, leaky=leaky),  # B, 1024, T/4
            nn.AdaptiveAvgPool1d(1), # B, 1024, 1
            nn.Conv1d(1024, 1, kernel_size=1, stride=1, bias=True)  # B, 1, 1
        )
