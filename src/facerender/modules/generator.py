...
from src.facerender.modules.cross_attention import CrossAttentionSeq
...
class OcclusionAwareSPADEGenerator(nn.Module):
    def __init__(
        ...
        self.fusion = fusion is not None
        if self.fusion:
            # self.attn = CrossAttentionSeq(**fusion)
            self.proj1 = nn.Conv2d(fusion["feat_dim"], fusion["cor_dim"], 1)
            self.proj2 = nn.Conv2d(fusion["cor_dim"], fusion["feat_dim"], 1)
            self.linear = nn.Conv2d(
                in_channels=max_features, out_channels=max_features, kernel_size=1
            )
    ...
    def forward(self, source_image, kp_driving, kp_source, cor=None):
        # Encoding (downsampling) part
        out = self.first(source_image)
        for i in range(len(self.down_blocks)):
            out = self.down_blocks[i](out)
        ...
        bs, c, h, w = out.shape
        # print(out.shape)
        if self.fusion and cor is not None:
            cor = F.normalize(cor, p=2, dim=-1)
            out = out + self.proj2(torch.einsum("bcc,bchw->bchw", cor, self.proj1(out)))
            out = self.linear(out)
        ...
        # Decoding part
        # if self.fusion and cor is not None and fa is not None:
        #     b, c, h, w = out.shape
        #     out = out + self.attn(
        #         cor.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, h, w),
        #         fa.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, h, w),
        #         out,
        #     )
    ...
    def get_trainable_params(self):
        params = []
        if self.dense_motion_network is not None:
            params += list(self.dense_motion_network.parameters())
        if self.fusion:
            params += list(self.proj1.parameters()) + list(self.proj2.parameters())
        return params
    ...
    def train(self, mode=True):
        ...
        if mode:
            if self.dense_motion_network is not None:
                self.dense_motion_network.train()
            if self.fusion:
                self.proj1.train()
                self.proj2.train()
