...
def init_path(checkpoint_dir, config_dir, size=512, old_version=False, preprocess='crop'):
    ...
    if old_version:
        ...
        sadtalker_paths = {
            "checkpoint":os.path.join(checkpoint_dir, 'SadTalker_custom.safetensors'), # 'SadTalker_V0.0.2_'+str(size)+'.safetensors'
            }
        use_safetensor = True
    else:
    ...
    if 'full' in preprocess:
        ...
        sadtalker_paths['mappingnet_checkpoint'] = os.path.join(checkpoint_dir, 'mapping_custom.pth.tar') # 'mapping_00229-model.pth.tar'
