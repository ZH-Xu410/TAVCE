...
class CropAndExtract():
    ...
    def generate(self, input_path, save_dir, crop_or_resize='crop', source_image_flag=False, pic_size=256, save_img=True):
        ...
        if not os.path.isfile(landmarks_path): 
            lm = self.propress.predictor.extract_keypoint(frames_pil, landmarks_path, info=False)
        else:
        ...
        if not os.path.isfile(coeff_path):
            ...
            for idx in range(len(frames_pil)): #tqdm(range(len(frames_pil)), desc='3DMM Extraction In Video:'):
