...
def run(params):
    ...
    unexists = []
    chunks_data = []
    for video_id in tqdm(os.listdir(video_folder), desc=str(person_id)):
        intermediate_files = []
        try:
            video_path = os.path.join(args.video_folder, video_id + ".mp4")
    ...
    if len(unexists) > 0:
        with open('unexists/'+str(person_id)+'.txt', 'w') as f:
            f.write('\n'.join(unexists))
    return chunks_data
...
if __name__ == "__main__":
    ...
    parser.add_argument("--annotations_folder", default='~/datasets/VoxCeleb1/txt', help='Path to utterance annotations')

    parser.add_argument("--video_folder", default='~/datasets/VoxCeleb1/videos', help='Path to intermediate videos')
    parser.add_argument("--chunk_folder", default='~/datasets/VoxCeleb1/chunks', help="Path to folder with video chunks")
    parser.add_argument("--bbox_folder", default='~/datasets/VoxCeleb1/bboxes', help="Path to folder with bboxes")
    parser.add_argument("--out_folder", default='~/datasets/VoxCeleb1/images', help='Folder for processed dataset')
    parser.add_argument("--chunks_metadata", default='~/datasets/VoxCeleb1/metadata.csv', help='File with metadata')

    parser.add_argument("--youtube", default='./yt-dlp_linux', help='Command for launching youtube-dl')
...
# python crop_vox.py --workers 1 --device_ids 0 --format .png --dataset_version 1 --data_range 10000-11252 --remove-intermediate-results
# python crop_vox.py --workers 1 --dataset_version 1 --data_range 10000-11252 --no-split-in-utterance --no-estimate-bbox --no-crop
# python crop_vox.py --workers 8 --device_ids 0 --format .png --dataset_version 1 --data_range 10652-11252 --no-download --remove-intermediate-results
