...
def _single_process(data_list, fn, args):
    device_id = args.device_ids.split(",")[0]
    ...
    for data in data_list:
        chunks_data = fn((data, device_id, args))
...
def scheduler(data_list, fn, args):
    if args.workers <= 1:
        return _single_process(data_list, fn, args)
    ...
    line = "{video_id},{start},{end},{bbox},{fps},{width},{height},{partition}"
    print (line.replace('{', '').replace('}', ''), file=f)
    for chunks_data in pool.imap_unordered(fn, zip(data_list, cycle(device_ids), args_list)):
        for data in chunks_data:
            print (line.format(**data), file=f)
            f.flush()
    f.close()
